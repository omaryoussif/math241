...
ggplot(data = covid_survey_summary_stats, mapping = aes(mean, explanatory_value, xmin = low, xmax = high)) +
  ...
  geom_point() +
  facet_grid(vars(factor(explanatory, levels = c("All", "Age", "Gender", "Race", "Ethnicity", "Profession", "Had COVID Vaccine", "Had flu vaccine this year"))), vars(response), space = "free", scales = "free_y", labeller = labeller(response = label_wrap_gen(15))) +
  theme_minimal() + 
