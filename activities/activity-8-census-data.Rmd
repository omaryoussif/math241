...
```{r, eval = F}
census_api_key("789d6acd07a9d2e75d1edcc200969ebfc1c72596", install = TRUE)
```
...
or_county %>%
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = white_popE/total_popE)) +
  scale_fill_viridis_c() +
  labs(fill = "Race") +
  theme_void()
```
...
clackamas_county <- get_acs(geography = "county", 
              year = 2022,
              variables = c(house_val = "B25077_001"), 
              state = "OR",
              county = c("Multnomah", "Clackamas", "Washington", "Hood River", "Wasco", "Marion", "Yamhill"),
              survey = "acs5",
              output = "wide")

clackamas_county <- left_join(clackamas_county, or_sf, by = c("GEOID" = "GEOID"))
```
...
clackamas_county %>% 
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = house_valE)) +
  #geom_text_repel(aes(label = NAME.x, geometry = geometry), stat = "sf_coordinates", size = 3) +
  scale_fill_viridis_c() +
  labs(fill = "Median House Value") +
  theme_void()
```
